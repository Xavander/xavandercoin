...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("XCZM")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeXavanderAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeXavanderAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeXavanderAmount = strAnonymizeXavanderAmount.remove(strAnonymizeXavanderAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXavanderAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeXavanderAmount * COIN) nMaxToAnonymize = nAnonymizeXavanderAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeXavanderAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeXavanderAmount));
        strAnonymizeXavanderAmount = strAnonymizeXavanderAmount.remove(strAnonymizeXavanderAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeXavanderAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
